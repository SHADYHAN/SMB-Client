...
public sealed class MainShellViewModel : ObservableObject, IDisposable
{
    ...
    public FileDownloadResult PrepareSelectedItemForDragDownload()
    {
        if (_session is null || SelectedDirectoryItem is null)
        {
            return new FileDownloadResult(
        ...
        var item = SelectedDirectoryItem;
        if (_fileDragDownloadPreparationService is not null)
        {
            var prepared = _fileDragDownloadPreparationService.TryGetPrepared(item);
            if (prepared?.Succeeded == true && !string.IsNullOrWhiteSpace(prepared.LocalPath))
            {
                StatusText = $"{(item.IsDirectory ? "文件夹" : "文件")}已准备好，可拖出到本地：{item.Name}";
                return prepared;
            }

            _ = PrepareDragDownloadForSelectionAsync(automatic: false);
            StatusText = item.IsDirectory
                ? $"正在准备拖出文件夹：{item.Name}，请稍后再拖。"
                : $"正在准备拖出文件：{item.Name}，请稍后再拖。";
            return new FileDownloadResult(
                false,
                item.IsDirectory
                    ? $"正在准备拖出文件夹：{item.Name}，请稍后再拖。"
                    : $"正在准备拖出文件：{item.Name}，请稍后再拖。",
                null,
                ErrorCode: "download.drag_not_ready"
            );
        }
        ...
        return new FileDownloadResult(
            ...
            "拖出下载准备服务不可用。",
            null,
            ErrorCode: "download.drag_service_unavailable"
        );
    }
