...
public sealed partial class MainWindow : Window
{
    ...
    private void ClearDirectoryDragState()
    {
    ...
    private void DirectoryItem_DragStarting(UIElement sender, DragStartingEventArgs e)
    {
        if (sender is not FrameworkElement { DataContext: DirectoryItemViewModel item })
        {
            e.Cancel = true;
            return;
        }
        ...
        var draggedItems = selectedItems.ToArray();
        _activeInternalDragItems = draggedItems;
        ConfigureDirectoryDragData(e.Data, draggedItems);
        e.AllowedOperations = DataPackageOperation.Copy;
        e.DragUI.SetContentFromDataPackage();
    }

    private void ConfigureDirectoryDragData(
        DataPackage dataPackage,
        IReadOnlyList<DirectoryItemViewModel> draggedItems
    )
    {
        dataPackage.SetData(InternalDirectoryDragFormat, "1");
        dataPackage.Properties.Title = BuildDragTitle(draggedItems);
        dataPackage.Properties.Description = "RYNAT 共享网盘";
