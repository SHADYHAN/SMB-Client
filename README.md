...
```text
http://127.0.0.1:19527/s?d=eyJoIjoiMTkyLjE2OC4xMDIuMTM2IiwicyI6IkJhY2tvZmZpY2UiLCJwIjoiL0NvbnRyYWN0cy8yMDI0IiwidCI6ImRpciJ9
http://127.0.0.1:19527/s?d=eyJoIjoibmFzLmxvY2FsIiwicyI6Ik1lZGlhIiwicCI6Ii9Nb3ZpZXMvZGVtby5tcDQiLCJ0IjoiZmlsZSJ9
```
...
```text
rynat://s?d=eyJoIjoibmFzLmxvY2FsIiwicyI6Ik1lZGlhIiwicCI6Ii9Nb3ZpZXMvZGVtby5tcDQiLCJ0IjoiZmlsZSJ9
```

参数：`d` 是无填充 base64url 编码的固定 JSON payload，内部字段为 `h` 服务器地址、`s` 共享名、`p` 共享内路径、`t` `file`/`dir`。中文共享名和路径不会在链接里展开成很长的 `%E5...` 转义串。
