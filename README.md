...
apps/windows/           Windows WPF 后备客户端（C# / .NET 8，已归档为 fallback）
...
- **原生 UI** — macOS 使用 AppKit。Windows WPF 客户端已归档为可构建后备线；新的 Windows 主方向转为 Explorer-first，由 RYNAT 负责登录、链接和 Shell 集成，把文件浏览 / 复制 / 移动 / 预览交给 Windows Explorer。
...
- Windows WPF 客户端已完成阶段性功能验证并归档为后备线；不再作为 Windows 主体验继续投入 UI / 文件管理器精修。
- Windows 下一阶段主方向切到 Explorer-first：RYNAT 提供登录、服务器设置、凭据接入、Explorer 右键复制分享链接、链接唤醒、托盘 / 诊断，文件浏览和文件操作交给 Windows Explorer。
- 旧 WinUI 3 版本只保留在 `apps/windows-winui-legacy` 作为历史参考；当前 WPF 后备线保留在 `apps/windows`，用于回退、调试和复用登录 / 链接 / platform adapter 代码。
