...
- 登录页保持简约，只负责默认服务器下的用户名、密码、记住密码、自动登录和登录动作。
- 登录页不做服务器下拉、不做服务器编辑；默认服务器在登录后的“服务器”页管理。
- 登录后的主界面保留“服务器”和“通用”两栏。“服务器”负责服务器列表、新增、删除、默认服务器；“通用”承载快捷键、开机启动、右键菜单状态和后续设置项。
- 本地密码保存只面向当前 Windows 用户，使用 Windows DPAPI 保护，不写入明文配置。
...
用户打开 RYANT共享网盘 后先进入登录页。登录成功后，RYNAT 将 SMB 访问能力交给 Windows 系统，并自动打开 Explorer 到共享根或上次目录。
...
服务器地址和 profile 管理放在登录后的“服务器”设置页，不放在登录页。登录页只显示当前默认服务器名称和地址，保持与 macOS 端一致的简约入口。
...
- Explorer 右键“复制分享链接”可用，支持文件和文件夹。
