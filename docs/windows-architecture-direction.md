...
> 2026-06-27 archive note: This document records the WPF file-workbench phase and the reasoning that led away from the old WinUI 3 implementation. The WPF client under `apps/windows` is now archived as a buildable fallback / reference line. The active Windows direction is Explorer-first; see `docs/explorer-first-windows-client.md`.
...
WPF 后备线不删除，但只保留为可构建后备、调试入口和可复用代码来源。它的阶段性价值是验证了三件核心事：
...
WPF 曾是替代旧 WinUI 3 的主线选择，原因是它比旧 WinUI 代码更适合快速建立稳定的桌面文件工作台。但在内部验证后，当前决策是将 WPF 归档为 fallback，而不是继续投入 UI / Explorer 复刻精修。
...
- 保持可构建、可启动。
- 作为登录 / 链接 / platform adapter 的参考来源。
- 作为 Explorer-first 未完成前的回退版本。
- 只修阻断构建、安全和关键链接问题，不继续扩张文件管理器体验。
...
2026-06-30 技术栈更新：Explorer-first 主程序从 Tauri 评估线切换为 `.NET 10 WinForms Tray Host + WebView2 Local UI`。Tauri 保留为实验 / 参考线，不再作为当前主产品主线。
...
当前 `apps/windows` 是已归档的 WPF fallback 客户端，不再是 Windows 主体验。旧 WinUI 版本保留在 `apps/windows-winui-legacy`，只作为业务流程和问题样本参考，不应继续作为最终客户端基础。
...
5. Explorer-first 开始后，优先复用其中的登录、服务器设置、链接、protocol、local redirect、single instance 和 platform adapter 代码。
...
当前 WPF 后备线已经把本地 HTTP 中转、协议注册、单实例转发放入 `Platform/Activation`，Explorer-first 复用这些能力时应继续保持这个边界。
...
本阶段是 WPF 文件工作台时期的精修计划。Explorer-first 决策后，不再继续把 WPF 视觉精修作为 Windows 主产品路线；以下内容仅保留为历史记录和 fallback 修补参考。
...
WPF 文件工作台已完成内部验证并归档后，Explorer-first 成为当前 Windows 主线。
...
- 是否以单当前服务器模型降低复杂度。
- 主程序采用 `.NET 10 WinForms Tray Host + WebView2 Local UI`，Tauri 仅保留为实验 / 参考线。
- 是否保留 WPF 作为 fallback / reference。
...
- Explorer-first 轻壳采用 .NET WinForms 托盘宿主 + WebView2 本地 UI；Tauri 保留为实验 / 参考线。Explorer 右键入口仍应采用 Windows Shell 薄集成。
...
- WPF fallback 保持可构建、可启动，不再承载主文件管理体验。

## 总结

Windows 端的长期方向已调整为：保留 Rust core 和共享业务协议，把 WPF 客户端归档为可构建 fallback，将主体验切到 Explorer-first。
