...
- 生成固定 HTTP 分享链接并复制到剪贴板；Windows 侧纯文本保留 HTTP，富文本 href 附带 `rynat://` 直达链接。
- `rynat://` 协议注册、本地 HTTP 中转、单实例转发、前台唤醒的链接链路。
- 短链接 `/s/<code>`，以及本地已激活后的 `204 No Content` 浏览器响应，避免依赖脚本关闭标签页。
...
3. 优化预览：图片缩略图、视频首帧；大视频已避免自动整文件缓存，缓存清理策略已接入，仍需 Windows 实用观察。
...
2. 链接唤醒定位目录 / 文件。
3. 图片 / 视频基础预览和视频播放。
4. 本地短链接已激活后的 `204 No Content` 响应。
