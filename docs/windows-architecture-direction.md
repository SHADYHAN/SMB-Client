...
旧 `apps/windows` WPF 客户端已从代码树移除，不再是 Windows 主体验。旧 WinUI 版本保留在 `apps/windows-winui-legacy`，只作为业务流程和问题样本参考，不应继续作为最终客户端基础。
