...
- 多选远端复制 / 移动 / 粘贴，以及同名覆盖确认的 WPF app 层基础流程。
- 软件内远端拖拽到目录目标移动 / 复制，跨共享或按 Ctrl 时走复制。
...
1. 在真实 Windows SMB 环境验证并打磨远端复制 / 移动 / 粘贴 / 拖拽，尤其是目录递归、跨共享限制和同名覆盖行为。
2. 打磨拖拽体验：软件内无效目标只做视觉反馈，拖到有效目录目标后才触发移动 / 复制；拖出桌面的视觉影子和同名行为继续贴近 Explorer。
3. 优化预览：图片预览已生成轻量缩略图，视频预览已接入 Windows Shell poster；大媒体已避免自动整文件缓存，缓存清理策略已接入，仍需 Windows 实用观察不同编码的 poster 命中率。
...
5. 继续保持 Windows 侧 smoke checks，当前已覆盖启动参数、本地中转、协议注册和单实例转发；后续新增 Shell 集成时同步补检查。
...
2. 视频 poster / 首帧和大文件预览性能；图片预览已使用轻量缩略图，视频预览已接入 Windows Shell poster。
3. 链接在钉钉、飞书、浏览器、客户端冷启动 / 热启动下的持续实测。
4. 可选浏览器扩展方案：Chrome / Edge MV3 扩展监听 `http://127.0.0.1:19527/s/*` 和 `http://localhost:19527/s/*`，命中后延迟约 500-800ms 调用 `chrome.tabs.remove(tabId)` 关闭中转标签页。该方案只适用于安装了扩展的浏览器，无法覆盖钉钉内置 WebView；默认复制链接仍保持 HTTP，避免破坏钉钉文档 / 聊天可点击性。
