...
定位为 RYNAT 文件工作台
优先打磨链接、预览缩略、上传下载
复杂 Shell / Explorer 复刻能力降级为可选
```

不建议继续把旧 WinUI 3 版本作为长期成品基础。它可以作为功能验证参考，但不应继续堆补丁到最终版本。

WPF 主线继续维护，但目标不是替代 Windows Explorer。它的核心价值是让用户更快完成三件事：

1. 复制和打开分享链接。
2. 通过缩略图和预览快速判断文件内容。
3. 稳定上传、下载和处理常见文件进出流程。

目录树、文件列表、多选、右键、拖拽、搜索、状态栏和预览面板，都是这三件核心能力的易用性延伸。超出这三件事的重型资源管理器能力，只有在明确提升用户体验且不显著损害稳定/性能时才保留。
...
- 简约、清晰、精致，接近 mac 端主界面的克制气质。
- 不追求 Explorer 所有系统能力。
- 文件列表用于快速找文件、选文件、复制链接、预览和上传下载。
- 文件操作能力以稳定和易懂为第一优先级。
...
这些地方继续修可以改善局部，但很难变成稳定的资源管理器体验。新的 WPF 主线应回到 RYNAT 自身价值：链接、预览缩略、上传下载。

### 后期：Explorer-first / Shell 集成

如果后续发现用户更需要系统文件管理器体验，可以另开 Explorer-first 方向：RYNAT 负责登录、右键复制链接和链接唤醒，文件浏览和复制移动交给 Windows Explorer。

该方向已单独记录在 `docs/explorer-first-windows-client.md`，不应和当前 WPF 文件工作台路线混在一起推进。
...
- 多选远端复制 / 移动 / 粘贴，以及同名覆盖确认的 WPF app 层基础流程；该能力保留为可用增强，不再作为核心体验继续扩张。
...
1. 链接链路稳定：复制 HTTP 分享链接、钉钉 / 聊天 / 文档可点击、本地中转、单实例唤醒、未登录后继续消费待处理链接。
2. 预览缩略稳定：图片轻量缩略图、视频 poster / 首帧、播放入口、快速切换取消、缓存清理和大文件保护。
3. 上传下载稳定：拖入上传、下载到本地、任务进度、取消、失败提示、同名确认。
4. UI 视觉精修：对齐 mac 端简约精美气质，统一间距、圆角、字号、图标、hover / selected / pressed 状态和预览舞台。
5. 控制复杂度：远端复制 / 移动 / 粘贴、软件内拖拽、拖出桌面等复杂能力只保留稳定必要部分；若影响核心链路稳定、性能或维护成本，可降级或移除。
6. 继续拆薄 `ShellViewModel`。远端剪贴板已抽到 `RemoteClipboardCoordinator`，链接激活已抽到 `LinkActivationCoordinator`，目录导航已抽到 `DirectoryNavigationCoordinator`，预览加载已抽到 `PreviewCoordinator`，文件拖入/拖出协调已抽到 `FileDragDropCoordinator`；后续只在核心链路继续膨胀时再拆。
7. 继续保持 Windows 侧 smoke checks，当前已覆盖启动参数、本地中转、协议注册和单实例转发；后续对链接、预览、上传下载增加更贴近核心链路的检查。
...
Windows WPF 端不再以复刻资源管理器为目标，而以 RYNAT 文件工作台为目标。核心体验围绕链接、预览缩略、上传下载三条链路建立。
...
### 阶段六：Explorer-first 评估（独立方向）

仅当 WPF 文件工作台已稳定，但用户仍强烈需要系统文件管理器体验时进入。

评估项：

- 是否登录后直接打开 Windows Explorer。
- 是否注册 Explorer 右键复制分享链接。
- 是否以单当前服务器模型降低复杂度。
- 是否保留 WPF 作为登录 / 设置 / 诊断 / fallback 界面。
...
- 可连接真实 SMB 服务器。
- 复制分享链接和链接唤醒稳定。
- 链接在钉钉文档 / 聊天框、浏览器、客户端冷启动 / 热启动下可用。
- 图片 / 视频缩略和预览可用，预览不阻塞目录操作。
- 上传、下载稳定，长任务有状态、失败提示和取消能力。
- 删除、重命名、新建文件夹作为基础管理能力可用。
- 同名冲突必须让用户确认。
- 窗口大小自由调整，布局自适应。
- UI 视觉达到简约、清晰、精致：间距、圆角、字号、图标、状态层次统一。
- 复杂拖拽、远端复制 / 移动、Shell 深集成可以降级或移除，不影响核心链路。
- 代码结构中不存在超大主窗口类和超大 ViewModel。

## 总结

Windows 端的长期方向应是：保留 Rust core 和共享业务协议，继续维护 WPF 主客户端，但把它定位为 RYNAT 文件工作台，而不是 Explorer 平替。

当前路线优先打磨三条核心链路：链接、预览缩略、上传下载。目录树、文件列表、右键、拖拽和搜索都是这些核心能力的辅助。任何复杂功能只要影响稳定、性能或维护成本，都可以降级或移除。

Explorer-first 和 Cloud Files 应作为独立方向评估，不和当前 WPF 文件工作台路线混在一起推进。
