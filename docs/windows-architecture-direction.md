...
Windows 端已经有完整实现，但当前 UI / app 层更像是在 WinUI 3 上手工模拟资源管理器。实际体验中已经暴露出拖拽影子、拖出桌面临时路径、同名覆盖、目录树交互、文件列表列宽、选择状态等一系列问题。
...
本文档用于确定 Windows 端长期方向。原则是保留 Rust core，重新设计 Windows 原生客户端层，不继续在当前 WinUI 3 主窗口结构上无限打补丁。
...
不建议继续把当前 WinUI 3 版本作为长期成品基础。它可以作为功能验证参考，但不应继续堆补丁到最终版本。
...
建议 Windows 桌面主客户端使用 WPF，而不是继续基于当前 WinUI 3 版本推进。
...
- `rynat://` 协议注册、本地 HTTP 中转、单实例转发的链接唤醒基础链路。

当前 WPF 主线的剩余重点：

1. Windows 实机确认构建、启动、登录、目录浏览和基础文件操作稳定。
2. 链接功能闭环实测：浏览器、钉钉文档、客户端已打开、客户端未打开四种场景。
3. 补齐软件内复制 / 移动 / 粘贴，以及同名冲突确认。
4. 打磨拖拽体验：软件内拖动只做视觉反馈，跨目录后才触发移动 / 复制；拖出桌面的视觉影子要接近系统表现。
5. 优化预览：图片缩略图、视频首帧、大视频避免整文件缓存、缓存清理策略。
6. 继续拆薄 `ShellViewModel`，防止新的 WPF 主线再次形成大控制器。
...
目标：建立新 Windows 客户端工程骨架，不追求全部功能。

内容：

1. 新建 WPF app。
2. 接入 Rust core bridge。
3. 建立 DI / service registry。
4. 实现登录页。
5. 实现服务器配置读取。
...
4. 当前 WinUI ListView 目录树模拟逻辑。
5. 当前 WinUI DataPackage 拖出逻辑。
6. 任何直接依赖 WinUI 控件的 service 入参。

## 不建议做的事

- 不建议继续在当前 WinUI 主窗口上修视觉和拖拽细节作为长期方案。
...
Windows 端的长期方向应是：保留 Rust core 和共享业务协议，重新设计 Windows 原生客户端层。第一阶段建议采用 WPF 构建主客户端，以更成熟可控的桌面控件和 Win32/Shell 互操作承载文件管理器体验。当前 WinUI 3 代码可作为功能验证和业务流程参考，但不建议作为最终成品架构继续补丁化推进。
