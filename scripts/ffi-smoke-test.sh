...
pwsh -ExecutionPolicy Bypass -File "$ROOT_DIR/scripts/check-bridge-surface.ps1"
...
static int call_and_print(const char *name, char *(*fn)(const char *), const char *json) {
    char *output = fn(json);
    if (output == NULL) {
...
int main(void) {
    ...
    status |= call_and_print(
        ...
        "{\"raw_link\":\"rynat://s?h=files.example&s=Team&p=/Project/file.pdf&t=file\"}"
    );
    status |= call_and_print(
        "preview",
