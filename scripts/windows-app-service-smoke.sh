...
assert_file_contains "$windows_app" 'new SmbTaskService\(bridge\)' 'SmbTaskService registered'
...
assert_file_contains "$windows_app" 'new FileOperationService\(taskService\)' 'FileOperationService registered'
assert_file_contains "$windows_app" 'new FileTransferService\(taskService\)' 'FileTransferService registered'
...
assert_file_contains "$core_bridge" 'public SmbTaskStatus SmbPollTask' 'C# bridge wraps task poll'
assert_file_contains "$core_bridge" 'public SmbTaskStatus SmbCancelTask' 'C# bridge wraps task cancel'
assert_file_contains "$core_bridge" 'public void SmbClearTask' 'C# bridge wraps task clear'
...
assert_file_contains "$login_coordinator" 'EnsureLoginProfileAsync' 'manual login saves a profile id before connecting'
...
assert_file_contains "$login_coordinator" 'loginProfile\?\.Id' 'manual login uses stable profile connection id'
...
assert_file_contains "$login_view_model" 'Password = string\.Empty' 'login profile switching clears typed password'
assert_file_contains "$login_view_model" 'Username = value\.Username \?\? string\.Empty' 'login profile switching avoids carrying stale usernames'
assert_file_contains "$ROOT_DIR/apps/windows/UI/Login/LoginView.xaml.cs" 'SyncPasswordBox' 'login view syncs PasswordBox after view-model password clears'
...
assert_file_contains "$directory_navigation_coordinator" 'directoryLoadGeneration' 'directory navigation coordinator guards against stale load completion'
assert_file_contains "$directory_navigation_coordinator" 'CancelActiveDirectoryLoad' 'directory navigation coordinator cancels in-flight load when switching roots'
assert_file_contains "$directory_navigation_coordinator" 'handleSessionIssueAsync' 'directory navigation coordinator routes reconnectable failures to shell'
...
assert_file_contains "$file_drag_drop_coordinator" 'handleOperationResultAsync' 'drag/drop coordinator routes reconnectable operation results'
...
assert_file_contains "$preview_coordinator" 'CancellationTokenSource' 'preview coordinator cancels stale preview work'
assert_file_contains "$preview_coordinator" 'handleSessionIssueAsync' 'preview coordinator routes reconnectable failures to shell'
...
assert_file_contains "$file_list_view" 'e\.Data\.GetDataPresent\(RemoteDragPayload\.DataFormat\)' 'file list does not re-upload internal drag-out FileDrop payloads'
...
assert_file_contains "$file_operation_service" 'SmbTaskOperation\.CreateDirectory' 'file operation service tasks core create-directory'
assert_file_contains "$file_operation_service" 'SmbTaskOperation\.Delete' 'file operation service tasks core delete'
assert_file_contains "$file_operation_service" 'SmbTaskOperation\.Rename' 'file operation service tasks core rename'
assert_file_contains "$file_operation_service" 'SmbTaskOperation\.UploadFile' 'file operation service tasks core upload'
assert_file_contains "$file_operation_service" 'RunWriteTaskAsync' 'file operation service runs writes through core task API'
assert_file_not_contains "$file_operation_service" 'GetAwaiter\(\)\.GetResult' 'file operation service does not block on async task polling'
...
assert_file_not_contains "$remote_copy_move_service" 'DeleteRemotePathRecursive|DeleteTargetIfExists|replace-delete' 'remote copy/move service avoids delete-before-replace data loss'
assert_file_contains "$remote_copy_move_service" 'directory_replace_not_supported' 'remote directory replace is rejected instead of deleting target first'
assert_file_contains "$remote_copy_move_service" 'move_replace_not_supported' 'remote move replace is rejected instead of deleting target first'
...
assert_file_not_contains "$file_transfer_service" 'LazyRemoteDownloadStream' 'drag-out no longer downloads lazily during DoDragDrop'
assert_file_contains "$file_transfer_service" 'SmbTaskOperation\.CacheFile' 'drag-out pre-caches through core task API'
...
assert_file_contains "$file_transfer_service" 'LocalPath' 'drag-out payload exposes cached local file path'
assert_file_contains "$shell_drag_drop_service" 'FileGroupDescriptorW' 'drag-out keeps virtual file descriptor fallback'
assert_file_contains "$shell_drag_drop_service" 'FileContents' 'drag-out keeps virtual file contents fallback'
assert_file_contains "$shell_drag_drop_service" 'DataFormats\.FileDrop' 'drag-out publishes cached local file path to Explorer'
...
assert_file_contains "$preview_service" 'SmbCancelOperation' 'preview service cancels native cache operation on preview cancellation'
...
assert_file_contains "$windows_app" 'ShutdownAsync\(\)' 'app exit disconnects active SMB session'
